...
if [ ! -e linux/Makefile ]; then
	...
	cd ${OE_BASE}/build-${DISTRO} &&
		source env.source &&
		${OE_BASE}/bb/bin/bitbake linux-omap419 -cclean &&
		${OE_BASE}/bb/bin/bitbake linux-omap419 -cconfigure && {
			cd ${curdir}
			echo
			echo "Copy linux sources..."
			echo
			cp -R ${OE_BASE}/build-${DISTRO}/tmp/work/board-tv-linux-gnueabi/linux-omap419-*/linux-*/ linux/
