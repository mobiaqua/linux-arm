...
if [ ! `echo ${PATH} | grep aarch64-softvm-linux` ]; then
	export PATH=${SYSROOT_NATIVE}/usr/bin/:${SYSROOT_NATIVE}/usr/bin/aarch64-softvm-linux:${PATH}
fi
...
export CROSS_COMPILE=aarch64-softvm-linux-
