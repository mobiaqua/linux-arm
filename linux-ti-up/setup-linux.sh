...
if [ ! -e linux/Makefile ]; then
	...
	cd ${OE_BASE}/build-${DISTRO}-${TARGET} &&
		...
		${OE_BASE}/bitbake/bin/bitbake linux-ti-up -cconfigure && {
			...
			echo
			cp -R ${OE_BASE}/build-${DISTRO}-${TARGET}/tmp/work-shared/beagle64/kernel-source/ linux/
			cp ${OE_BASE}/build-${DISTRO}-${TARGET}/tmp/work-shared/beagle64/kernel-source/.kernel-meta/configs/defconfig linux/.config
